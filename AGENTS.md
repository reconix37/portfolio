...
Даня посмотрел сайт после v3 и сказал: (1) Stack и Education — «нормисные», скучные таблицы; (2) редбулл в метриках убрать. Выполни Шаги 11-13 ниже, каждый отдельным коммитом.
...
### Шаг 13. УБРАТЬ РЕДБУЛЛ из метрик (WHOAMI)

Даня: «слишком много шуток про редбул и кофе»:
- `24/7 ALWAYS ON DUTY · RED BULL` → `24/7 ALWAYS ON DUTY` (просто, без напитка)
- Плашка рядом с маскотом `99% RED BULL` → убрать или заменить на `OS-TAN™` (Даня предложил) — решай, но НЕ про напитки
- Проверить, что нигде больше нет red bull / coffee / энергетиков в текстах сайта (только в этих двух местах были)
