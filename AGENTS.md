...
### Шаг 14. МИНИ-ТЯН-СТИКЕР на не-WHOAMI вкладках (как деплой-тян у KISA)

Даня хочет: маленький квадратик-стикер с маскотом, который появляется на вкладках PROJECTS/STACK/EDUCATION/CONTACT в углу — как деплой-тян плавает у KISA.

- Компонент `src/components/tabs/MiniMascot.tsx`: квадратный стикер ~72-96px, рамка border-2, hard shadow (`4px 4px 0 var(--line)`), фон bg-bg (как поляроид у WHOAMI)
- Внутри — спрайт маскота (тот же `public/mascot/mascot-*.png`, object-fit: cover, обрезка по лицу/верху — кропнуть так, чтобы видно было лицо и плечи, не весь рост)
- **Mood по вкладке** (реакция маскота): PROJECTS → happy, STACK → idle, EDUCATION → skeptical, CONTACT → surprised (как hover-реакции в PROJECTS)
- **Поведение**: появляется с fade/slide-in при переключении на не-WHOAMI вкладку (300ms), idle-покачивание как у большого маскота (медленный translateY, 3-4s)
- **Расположение**: фиксированный угол — правый нижний (нижний-правый внутри панели, absolute), НЕ перекрывает контент (z-index ниже, чем модалки; на мобиле — скрыть или уменьшить до 56px)
- Параллакс за курсором — лёгкий (как у большого Mascot), но необязательно; можно просто покачивание
- `prefers-reduced-motion` — без анимаций, стикер просто виден
- НЕ трогать существующий hero/Mascot.tsx и FloatingMascot — это отдельный элемент
