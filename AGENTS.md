...
4. **Маскот** — сейчас пиксельный кот-робот (PNG-спрайты в `public/mascot/`: `mascot-idle.png`, `mascot-happy.png`, `mascot-surprised.png`, `mascot-skeptical.png`), будет ЗАМЕНЁН на аниме-тян (спрайты придёт от Дани, те же имена файлов). Компонент `src/components/hero/Mascot.tsx` — проп `mood: 'idle'|'happy'|'surprised'|'skeptical'`, кроссфейд при смене, параллакс за курсором. Компонент абстрактный — менять только если поменяется API. См. CURRENT TASK п.6 (сцена вокруг маскота, сам маскот не трогать).
...
### 6. Маскот: ожидается замена на аниме-тян
Кот-робот будет ЗАМЕНЁН на аниме-тян (спрайты придёт от Дани позже, в `public/mascot/`, те же имена файлов: `mascot-idle.png`, `mascot-happy.png`, `mascot-surprised.png`, `mascot-skeptical.png`). Компонент `hero/Mascot.tsx` УЖЕ абстрактный (mood → картинка) — его НЕ менять, просто подложат новые файлы. НО: подготовь сцену вокруг маскота (как у KISA — книги/артефакты вокруг деплой-тян): 2-3 мини-элемента рядом с маскотом в hero — маленькие окошки-стикеры с `HABITFORGE`, `RAG`, `EVALS` (brutalist-стиль, border 2px, hard shadow, мелкий моноширинный текст). Это добавит «мир» вокруг персонажа. Маскот как таковой НЕ трогать и не «улучшать».
